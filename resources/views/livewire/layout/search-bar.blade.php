<div x-data="{ search: '', open: false, categories: @entangle('categories'), ads: @entangle('ads')  }"
    ...
    <input tabindex="2" name="search" wire:keydown.enter="performSearch" wire:model.live.debounce.300ms="search" x-model="search" type="text"
    ...
    <button type="button" x-bind:disabled="!open" aria-label="{{__('messages.t__aria_label_search')}}"  x-tooltip="{
        ...
        <x-icon-search aria-hidden="true" class="w-5 h-5 p-[1px]" />
    </button>
    <!-- Dropdown -->
    <div x-show="open"
        class="absolute w-full mt-1 bg-white shadow-lg ring-1 ring-gray-950/5 dark:ring-white/10  rounded-xl z-10 h-[70vh] md:h-auto overflow-auto  classic:ring-black dark:bg-gray-800 {{$homeSettings->enable_hover_animation?'classic:shadow-custom':''}}"
        x-cloak>
