<div class="relative">
    ...
    <div class="flex justify-between items-center gap-x-3">
        ...
        <nav aria-label="{{__('messages.t_aria_label_ad_type_navigation') }}" class="category-bar pt-6 overflow-x-auto hide-scrollbar md:max-w-[90%]"
        x-data="{
            ...
            <div class="flex md:space-x-4 whitespace-nowrap items-center">
                ...
                <div class="hidden md:block group">
                    <a
                        href="#" style="font-size: {{$homeSettings->all_category_font_size}}px"
                        class="flex gap-x-1 items-center  font-bold rounded-full py-2 w-24 md:w-auto">
                        {{ __('messages.t_all') }}
                        <x-heroicon-o-chevron-down class="w-6 h-6 transform {{$homeSettings->show_all_category_animation?'group-hover:rotate-180':''}}  transition-transform dark:text-gray-500" />
                    </a>
                    @if ($homeSettings->show_all_category_animation)
                    <div class="mega-group-container md:group-hover:block hidden absolute left-0 right-0 z-20 mt-0 overflow-auto h-[calc(100vh - 50rem)] bg-white shadow-lg rounded-xl ring-1 ring-gray-950/5  dark:bg-gray-900 dark:ring-white/10 classic:ring-black  classic:group-hover:shadow-custom md:group-focus-within:block">
                        <div class="flex mt-0">
                            @if ($isAdType)
                            <div class="cursor-pointer min-w-[20%] border-[#d1d5d8] classic:border-black border-r">
                                @foreach($adTypes as $adType)
                                <button type="button" class=" hover:bg-[#F1F1F1] {{ $selectedAdType?->id == $adType?->id ? 'bg-[#F1F1F1]' : '' }} p-3 flex justify-between items-center gap-x-7 cursor-pointer w-full !-outline-offset-4"
                ...
                <button type="button" x-show="!isScrollStart"
                class=" bg-white/80 " @click="scrollLeft" >
                <x-heroicon-o-chevron-left x-tooltip="{
                ...
                @foreach($adTypes as $adType)
                <li class="group" >
                    <a  draggable="false" href="{{ generate_category_url($adType, null) }}" class="block md:text-sm md:py-1 md:px-3  border border-transparent dark:border-gray-900 md:rounded-full  md:group-hover:border-black dark:group-hover:border-white/10 transition-all md:group-hover:transform md:group-hover:-translate-x-1 md:group-hover:-translate-y-1 classic:border-b-4 classic:border-r-4 classic:border-transparent classic:border-black w-24 md:w-auto mx-1" >
                        <img src="{{ $adType?->icon }}" alt="{{ $adType?->name }}" class="h-12 w-12 md:w-20 md:h-20 pb-3 mx-auto  md:hidden">
                        <span class="text-xs md:text-sm line-clamp-1 whitespace-normal md:line-clamp-none md:whitespace-nowrap text-center uppercase md:capitalize">{{ $adType?->name }}</span>
                    </a>
                </li>
                @endforeach
                @else
                ...
                <li class="group" >
                    <a  draggable="false"  href="{{ generate_category_url($category->adType, $category) }}" class="block md:text-sm md:py-1 md:px-3  border border-transparent dark:border-gray-900 md:rounded-full  md:group-hover:border-black dark:group-hover:border-white/10 transition-all md:group-hover:transform md:group-hover:-translate-x-1 md:group-hover:-translate-y-1 classic:border-b-4 classic:border-r-4 classic:border-transparent classic:border-black w-24 md:w-auto mx-1" >
            ...
            <x-heroicon-o-chevron-right x-tooltip="{
        ...
        <!-- Post your ad -->
        <div class="justify-end pt-6">
            <a href="/post-ad"
                class=" block text-white py-2 px-4 rounded-xl bg-black dark:bg-primary-600 dark:text-white whitespace-nowrap">
