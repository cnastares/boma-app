...
function sendTempMessageCard(message, id) {
 ...
 <div class="message-card mc-sender" data-id="${id}">
     ...
     <div class="flex items-center justify-end space-x-3 rtl:space-x-reverse !mt-1.5" dir="ltr">
		<svg class="text-gray-500 dark:text-zinc-300 w-4 h-4 -mt-px" stroke="currentColor" fill="currentColor" stroke-width="0" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><g><path fill="none" d="M0 0h24v24H0z"></path><path d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"></path></g></svg>
	 </div>
 </div>
`;
}

function sendTempMessageBuyerCard(message, id) {
    return `
    <div class=" pb-5 w-full mc-sender" data-id="${id}">
        <h2 class=" text-xl text-center font-semibold">Your Offer: ${message}</h2>
...
function IDinfo(id) {
        ...
        $.ajax({
            ...
            success: (data) => {
                ...
                if (data.user_avatar && data.user_avatar !== "") {
                    // If user_avatar is not empty, set it as the background image
                    $(".messenger-infoView").find(".avatar").css("background-image", 'url("' + data.user_avatar + '")');
                } else {
                    // If user_avatar is empty, apply style and set the first letter of the user's name
                    var firstLetter = data.fetch.name ? data.fetch.name.charAt(0) : '';
                    $(".messenger-infoView").find(".avatar")
                        .css({
                            "align-items": "center",
    ...
    canReceiveMessage();
                ...
                // update info in view
                $("#selected-contact-fullname").text(data.fetch.name);

                $("#selected-contact-view-profile-link").attr('href', '/profile/' + data.fetch.slug + '/' + data.fetch.id);

                $("#selected-contact-fullname").show(data.fetch.name);

                $(".messenger-infoView .info-name").text(data.fetch.name);

                $(".m-header-messaging .user-name").text(data.fetch.name);
                if(data.ad_url){
                    $(".m-header-messaging #ad-link").attr('href',data.ad_url);
                }else{
                    $(".m-header-messaging #ad-link").hide();
                }
                // Star status
                data.favorite > 0
...
function sendMessage() {
    ...
    if (inputValue.length > 0 || hasFile || window.chatMessageHandler.audioBlob) {
        const formData = new FormData($("#message-form")[0]);
        formData.append("id", getMessengerId());
        formData.append("temporaryMsgId", tempID);
        formData.append("_token", csrfToken);
        // Check if there is an audio blob
        if (window.chatMessageHandler.audioBlob) {
            formData.append("audio", window.chatMessageHandler.audioBlob, "message_audio.webm");
        }

        let message = inputValue;

        if (isEnableOffer) {
            message = formData.set('message', `Negotiation` + '\n' + formData.get('message'));
            formData.append("is_negotiable_conversation", 1);
        }
        ...
        $.ajax({
            ...
            beforeSend: () => {
                ...
                if (hasFile || window.chatMessageHandler.audioBlob) {
                    messagesContainer
                        .find(".messages")
                        .append(
                            sendTempMessageCard(
                    ...
                    messagesContainer
                        .find(".messages")
                        .append(sendTempMessageCard(message, tempID));
                }
                // scroll to bottom
            ...
            success: (data) => {
                if (data.error > 0) {
                    ...
                    if (window.chatMessageHandler.audioBlob) {
                        // Clear message input, audio blob and reset timer
                        const chatHandlerEl = document.querySelector('[x-ref=chatHandler]');
                        chatHandlerEl.dispatchEvent(new CustomEvent('clear-data'));
                    }
                    // update contact item
...
function fetchMessages(id, newFetch = false) {
    ...
    if (id && $(window).width() < 768) {
        $('.messenger-listView').addClass('hidden');
    } else {
        $('.messenger-listView').removeClass('hidden');
    }
...
function cancelAttachment() {
    ...
    $(".messenger-chat-design").find(".attachment-preview").remove();
...
channel.bind("messaging", function (data) {
    ...
    // Always update contacts for any new message
    getContacts(true);
...
function getContacts(resetPagination = false) {
    // Reset pagination when updating for new messages
    if (resetPagination) {
        contactsPage = 1;
        noMoreContacts = false;
    }
    if (!contactsLoading && (!noMoreContacts || resetPagination)) {
...
function getSharedPhotos(user_id) {
    $.ajax({
        ...
        dataType: "JSON",
        success: (data) => {
            const sharedPhotosList = $(".shared-photos-list");

            sharedPhotosList.html(data.shared);

            // Check if data.shared has content and add class, else remove it
            if (data.empty > 0) {
                sharedPhotosList.addClass("grid grid-cols-4 md:gap-x-3 gap-y-4");
            } else {
                sharedPhotosList.removeClass("grid grid-cols-4 md:gap-x-3 gap-y-4");
            }
        },
        error: () => {
            console.error("Server error, check your response");
...
function deleteConversation(id) {
    $.ajax({
        ...
        success: (data) => {
            ...
            // Redirect to the main messages section
...
$(document).ready(function () {
    ...
    $("body").on("keypress", ".messenger-list-item", function (e) {
        if (e.key === "Enter") {
            if ($(this).find("tr[data-action]").attr("data-action") == "1") {
                $(".messenger-listView").hide();
            }
...
function canReceiveMessage(){
    ...
    $.ajax({
        url: "/check-can-message",
        method: "POST",
        data: { _token: csrfToken, seller_id: getMessengerId() },
        dataType: "JSON",
        success: (data) => {
            console.log(!data && data.can_message,!data.can_message);
...
function customMessage(value, isOffer = 0) {
    isEnableOffer = isOffer;
    $('#live-chat-message-textarea').html(value);
}
